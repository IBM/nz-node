...
const parseDbosDataRow = (reader, fieldCount, messageLength, tupdesc) => {
    ...
    // Helper function to navigate to a specific variable field using nzpy's algorithm
    // This matches CTable_i_varFieldPtr from nzpy
    const getVarFieldOffset = (varFieldIndex) => {
        if (!tupdesc || tupdesc.fixedFieldsSize === 0) {
            return dataStart;
        }
        
        // Start at the beginning of variable fields section
        let varOffset = tupdesc.fixedFieldsSize;
        
        // Skip through previous variable fields
        for (let ctr = 0; ctr < varFieldIndex; ctr++) {
            if (varOffset + 2 > tupleData.length) {
                break;
            }
            const length = tupleData.readUInt16LE(varOffset);
            // Skip the field data (length includes the 2-byte length prefix)
            if (length % 2 === 0) {
                varOffset += length;
            } else {
                // Odd length: skip length + 1 padding byte
                varOffset += length + 1;
            }
        }
    ...
    if (process.env.DEBUG_DATAROW && tupdesc) {
        console.log(`[DBOS DataRow] fixedFieldsSize=${tupdesc.fixedFieldsSize}, numVaryingFields=${tupdesc.numVaryingFields}`);
    }
    for (let i = 0; i < fieldCount; i++) {
        // Check if field is null using bitmap (bit=1 means NULL in Netezza)
        // CRITICAL: Use field_physField to map logical field to physical bitmap position
        const physField = tupdesc ? tupdesc.field_physField[i] : i;
        const byteIndex = Math.floor(physField / 8);
        const bitIndex = physField % 8;
        ...
        if (isNull) {
            ...
            // CRITICAL: For NULL fields, we don't advance varOffset because there's no data in the tuple
            // NULL fields don't take up space in the variable data section
            continue;
        }
            ...
            if (isFixedSize) {
                ...
                if (fieldType === netezza_types_1.NzTypeInt) {
                    ...
                    if (absoluteOffset + 8 <= tupleData.length) {
                        ...
                        // MINUS_INFINITY_MICROSECONDS = -2^63
                        const INFINITY_MICROSECONDS = Math.pow(2, 63) - 1;
                        const MINUS_INFINITY_MICROSECONDS = -Math.pow(2, 63);
                        
                        // When endtime is NULL in backend, Netezza sends 0 microseconds
                        // This should be treated as NULL, not as epoch date (2000-01-01)
                        if (micros === 0) {
                            fields[i] = null;
                        }
                        else if (micros === INFINITY_MICROSECONDS) {
                            fields[i] = 'infinity';
                        }
                        else if (micros === MINUS_INFINITY_MICROSECONDS) {
                            fields[i] = '-infinity';
                        }
                        else {
                            // Convert to JavaScript Date using UTC epoch (2000-01-01 00:00:00 UTC)
                            // Following nzpy's approach: EPOCH + microseconds
                            const epochUTC = Date.UTC(2000, 0, 1); // UTC epoch in milliseconds
                            const timestamp = new Date(epochUTC + micros / 1000);
                            // Convert to UTC ISO string for consistent timezone handling
                            fields[i] = timestamp.toISOString();
                        }
                    }
                    else {
                        fields[i] = null;
                ...
                // Variable-size field: use tuple descriptor offset to navigate
                // The field_offset tells us which variable field this is (0-indexed)
                const varFieldIndex = tupdesc.field_offset[i];
                const varOffset = getVarFieldOffset(varFieldIndex);
                
                if (process.env.DEBUG_DATAROW) {
                    console.log(`[DBOS DataRow] Field ${i}: varFieldIndex=${varFieldIndex}, varOffset=${varOffset}`);
                }
                ...
                if (fieldLength === 0 || fieldLength === 2) {
                    ...
                    // Note: We don't need to manually advance varOffset anymore
                    // because getVarFieldOffset calculates it fresh for each field
                    if (process.env.DEBUG_DATAROW) {
