    ...
    for (let i = 0; i < fieldCount; i++) {
        ...
        if (isNull) {
            ...
            if (isFixedSize) {
                ...
                if (process.env.DEBUG_DATAROW) {
                    console.log(`[DBOS DataRow] Field ${i}: reading from fixed offset ${absoluteOffset} (dataStart=${dataStart}, fieldOffset=${fieldOffset}, adjusted=${adjustedOffset})`);
                }
                // Parse based on type
                if (fieldType === netezza_types_1.NzTypeInt) {
                    ...
                    // NUMERIC/DECIMAL - variable-length, complex format
                    // For now, return null - proper implementation requires numeric.js module
                    fields[i] = null;
                }
                else {
