    ...
    parse(buffer, callback) {
        ...
        while (offset + NETEZZA_HEADER_LENGTH <= bufferFullLength) {
            ...
            if (fullMessageLength + offset <= bufferFullLength) {
                ...
                // Only call callback if message is not null (some messages are skipped)
                if (message !== null) {
                    callback(message);
                }
    ...
    handlePacket(offset, code, length, bytes) {
        ...
        switch (code) {
            ...
            case 68 /* MessageCodes.DataRow */:
                // Netezza sends DBOS format data rows with code 89 ('Y'), not 68 ('D')
                // Code 68 messages during Netezza connections are often metadata/version info
                // that should be skipped, especially if we have a DBOS tuple descriptor
                if (this.dbosTupleDesc) {
                    // Skip this message by reading remaining bytes and return null to continue parsing
                    reader.bytes(length - 4);
                    reader.setBuffer(0, emptyBuffer);
                    return null;
                } else {
                    // Try to parse as PostgreSQL DataRow, but skip if it fails
                    // (Netezza sometimes sends non-standard messages with code 68)
                    try {
...
const parseDbosDataRow = (reader, fieldCount, messageLength, tupdesc) => {
    ...
    for (let i = 0; i < fieldCount; i++) {
        ...
        const bitIndex = i % 8;
        let isNull = (bitmap[byteIndex] & (1 << bitIndex)) !== 0;
        
        // WORKAROUND: Netezza has a bug where fixed-size CHAR field bitmaps are incorrectly
        // set to NULL for active sessions, even though the data is present.
        // Check if data actually exists before treating as NULL.
        if (isNull && tupdesc && i < tupdesc.numFields) {
            const fieldType = tupdesc.field_type[i];
            const isFixedSize = tupdesc.field_fixedSize[i] === 1;
            const fieldSize = tupdesc.field_size[i];
            
            // For fixed-size CHAR fields, check if there's actual data
            if (isFixedSize && fieldType === netezza_types_1.NzTypeChar) {
                const fieldOffset = tupdesc.field_offset[i];
                const absoluteOffset = dataStart + fieldOffset;
                
                if (absoluteOffset + fieldSize <= tupleData.length) {
                    // Read the data and check if it's not all zeros/spaces
                    const data = tupleData.slice(absoluteOffset, absoluteOffset + fieldSize);
                    const hasData = data.some(byte => byte !== 0 && byte !== 0x20); // Not null byte or space
                    
                    if (hasData) {
                        // Override the NULL flag - there's actual data here
                        isNull = false;
                    }
                }
            }
            ...
            if (isFixedSize) {
                ...
                if (fieldType === netezza_types_1.NzTypeInt) {
                    ...
                    if (absoluteOffset + 8 <= tupleData.length) {
                        ...
                        // Handle special values (following nzpy's approach)
