...
const parseDbosDataRow = (reader, fieldCount, messageLength, tupdesc) => {
    ...
    for (let i = 0; i < fieldCount; i++) {
        ...
        // IMPORTANT: Use physical field index from tupdesc, not logical index
        // This matches nzpy's approach: bitmap[tupdesc.field_physField[field_lf]]
        const physFieldIndex = tupdesc ? tupdesc.field_physField[i] : i;
        const byteIndex = Math.floor(physFieldIndex / 8);
        const bitIndex = physFieldIndex % 8;
