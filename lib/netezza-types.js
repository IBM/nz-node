...
const toTimestamp = (val) => {
  ...
  // Check for sentinel values that indicate NULL
  // Netezza uses year 2000 as its epoch. When endtime is NULL in the backend,
  // it returns a sentinel value of 0 microseconds from epoch (2000-01-01),
  // which appears as "1999-12-31 18:30:00" UTC due to timezone offset.
  // Any timestamp before year 2000 is invalid and should be treated as NULL.
...
// Numeric/Decimal parser - returns as number
const toNumeric = (val) => {
  if (val === null || val === undefined) return null
...
// BigInt parser - returns as Number if safe, otherwise BigInt
const toBigInt = (val) => {
  if (val === null || val === undefined) return null
  ...
  // If already a bigint, convert to number if safe
  ...
  // Convert string or number to bigint first, then to number if safe
  try {
...
const PG_OID_TO_NZ_TYPE = {
  ...
  19: NzTypeVarChar,     // name type (treat as text)
  20: NzTypeInt8,        // int8/bigint
  21: NzTypeInt2,        // int2/smallint
  22: NzTypeVarChar,     // int2vector (treat as text)
  23: NzTypeInt,         // int4/integer
  25: NzTypeVarChar,     // text
  26: NzTypeInt,         // oid (treat as int)
  28: NzTypeInt,         // xid (treat as int)
  114: NzTypeJson,       // json
  700: NzTypeFloat,      // float4
  701: NzTypeDouble,     // float8
  702: NzTypeVarChar,    // abstime (treat as text - legacy type)
  705: NzTypeVarChar,    // unknown (treat as text)
  829: NzTypeVarChar,    // macaddr (treat as text)
  1000: NzTypeVarChar,   // bool[] (treat as text for now)
  1003: NzTypeVarChar,   // name[] (treat as text for now)
  1005: NzTypeVarChar,   // int2[] (treat as text for now)
  1007: NzTypeVarChar,   // int4[] (treat as text for now)
  1009: NzTypeVarChar,   // text[] (treat as text for now)
  1014: NzTypeVarChar,   // char[] (treat as text for now)
  1015: NzTypeVarChar,   // varchar[] (treat as text for now)
  1016: NzTypeVarChar,   // int8[] (treat as text for now)
  1021: NzTypeVarChar,   // float4[] (treat as text for now)
  1022: NzTypeVarChar,   // float8[] (treat as text for now)
  ...
  1231: NzTypeVarChar,   // numeric[] (treat as text for now)
  1263: NzTypeVarChar,   // cstring[] (treat as text for now)
  1266: NzTypeTimeTz,    // timetz
  1700: NzTypeNumeric,   // numeric/decimal
  2275: NzTypeVarChar,   // cstring (treat as text)
  2950: NzTypeVarChar,   // uuid (treat as text for now)
  3802: NzTypeJsonb,     // jsonb
  3614: NzTypeVarChar,   // tsvector (treat as text)
}
