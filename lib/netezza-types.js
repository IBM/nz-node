 ...
 * Based on Netezza type system, not PostgreSQL OIDs
 */
...
// Boolean parser - handles 't'/'f' or 1/0
const toBool = (val) => {
  if (val === null || val === undefined) return null
...
/**
 * Get type parser for a Netezza type
...
function getTypeParser(typeId, format = 'text') {
  ...
  if (defaultParsers[format] && defaultParsers[format][typeId]) {
    return defaultParsers[format][typeId]
  }
