...
// Timestamp parser - converts to UTC ISO string
// This ensures timestamps are always displayed in UTC regardless of local timezone
const toTimestamp = (val) => {
  if (val === null || val === undefined) return null
  
  // If it's already a Date object (from binary protocol), convert to UTC ISO string
  if (val instanceof Date) {
    return val.toISOString()
  }
  
  // If it's a string, parse and convert to UTC ISO string
  const date = new Date(val)
